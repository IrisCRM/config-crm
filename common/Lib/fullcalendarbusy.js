...
var defaults = {
	...
	columnFormat: {
/* // change: заголовок для масштаба месяца
		month: 'ddd', // like "Sat"
*/
		// change: рендеринг масштаба месяца
		month: "D\nddd", // like "2 Sat"
...
var fc = $.fullCalendarBusy = { version: "2.1.1" };
var fcViews = fc.views = {};


$.fn.fullCalendarBusy = function(options) {
	var args = Array.prototype.slice.call(arguments, 1); // for a possible method call
	var res = this; // what this function will return (this jQuery object by default)

	this.each(function(i, _element) { // loop each DOM element involved
		var element = $(_element);
		var calendar = element.data('fullCalendarBusy'); // get the existing calendar object (if any)
		...
		if (typeof options === 'string') {
			if (calendar && $.isFunction(calendar[options])) {
				...
				if (options === 'destroy') { // for the destroy method, must remove Calendar object data
					element.removeData('fullCalendarBusy');
				}
			}
		}
		// a new calendar initialization
			...
			element.data('fullCalendarBusy', calendar);
			calendar.render();
		}
	});
...
function EventManager(options) { // assumed to be a calendar
	...
	function buildEvent(data, source) { // source may be undefined!
		...
		//change: добавляем id пользователя в вывод дел
		if (data.user) {
			out.user = data.user;
		}
	...
	function mutateEvent(event, newStart, newEnd) {
		...
		if (newAllDay && viewName != 'irisWeek') {
			if (newStart) {
		...
		if (newAllDay != oldAllDay && viewName != 'irisWeek') {
	...
	function mutateEvents(events, clearEnd, forceAllDay, dateDelta, durationDelta) {
		...
		$.each(events, function(i, event) {
			...
			if (viewName == 'irisWeek') {
				if (!newEnd && event.end) {
					//newEnd = event.end.clone();
				}
				if (!newStart.hasTime()) {
...
GridCoordMap.prototype = {
	...
	computeBounds: function() {
		...
		if (this.containerEl) {
			...
			//change: высота считается не по 1 строке, а всего контейнера
			this.maxY = containerOffset.top + 
					this.containerEl.parents('tbody').outerHeight();
		}
	},
...
RowRenderer.prototype = {
	...
	//change: Для недели в заголовке добавляем строку с временем
	rowHtml: function(rowType, row) {
		...
		for (col = 0; col < view.colCnt; col++) {
			if (view.name == 'irisWeek' && rowType == 'head') {
				if (col % 4 == 0) {
					date = view.cellToDate(row, col);
					cellHtml += renderCell(row, col / 4, date);
				}
				timeRowHtml += '<th>' + hours[col % 4] + '</th>'
			}
			else {
				date = view.cellToDate(row, col);
...
$.extend(Grid.prototype, {
		...
		var dragListener = new DragListener(this.coordMap, {
			...
			cellOver: function(cell, date) {
				if (dragListener.origDate) { // click needs to have started on a cell
					...
					start = dates[0];
					//change: Для масштаба недели прибавляем только 6 часов
					if (view.name == 'irisWeek') {
						end = dates[1].clone().add(6, 'hours');
					}
					else {
	...
	headCellHtml: function(row, col, date) {
		...
		//change: colspan для недели
		var colspan = '';
		if (view.name == 'irisWeek') {
			colspan = ' colspan=4';
		}
		return '' +
			'<th' + colspan + ' class="fc-day-header ' + view.widgetHeaderClass + ' fc-' + dayIDs[date.day()] + '">' +
	...
	bgCellHtml: function(row, col, date) {
		...
		if (view.name == 'irisWeek') {
			return '<td class="' + classes.join(' ') + '" data-date="' + date.add(hours[col % 4], 'hours').format('YYYY-MM-DD[T]HH:mm:ss') + '"></td>';
		}

		//TODO:mnv: сюда бы лучше userid
	...
	getDayClasses: function(date) {
		...
		//change: добавлен striptime()
		if (date.stripTime().isSame(today, 'day')) {
			classes.push(
				'fc-today',
...
$.extend(Grid.prototype, {
	...
	eventToSegs: function(event, intervalStart, intervalEnd) {
		//change:
		var eventStart = event.start.clone(); // normalize
		var eventEnd = this.view.calendar.getEventEnd(event); // compute (if necessary) and normalize
		//if (this.view.name != 'irisWeek') {
		eventStart = eventStart.stripZone()
		eventEnd = eventEnd.stripZone()
		//}
		var segs;
		var i, seg;
	...
	segDragMousedown: function(seg, ev) {
		...
		var dragListener = new DragListener(view.coordMap, {
			...
			cellOver: function(cell, date) {
				...
				var res = _this.computeDraggedEventDates(cell, seg, origDate, date);
				newStart = res.start;
				newEnd = res.end;
				user = res.user;
...
*/
			...
			dragStop: function(ev) {
				...
				//change: если даты одинаковы, а пользователь нет, то тоже апдейт
				hasChanged = hasChanged || user != event.user;
				...
				mouseFollower.stop(!hasChanged, function() {
					...
					if (hasChanged) {
						event.user = user;
						//change: был глюк - иногда сбрасывалась дата конца
						event.end = newEnd;
	...
	//change: при драге смотрим еще и id пользователя
	computeDraggedEventDates: function(cell, seg, dragStartDate, dropDate) {
		//dropDate  = dropDate.add(1, 'month');
		var view = this.view;
		var event = seg.event;
		...
		var newStart;
		var newEnd;
		//var userid = seg.el.parents('td.fc-widget-content').prev('.fc-axis')
				//.data('user');
		//userid берем не из текущего seg, а из той строки, на которую собираемся плюхнуть элемент
		var userid = $(cell.grid.el[cell.row])
				.parents('td.fc-widget-content')
				.prev('.fc-axis').data('user');
		...
		return { start: newStart, end: newEnd, user: userid };
	},
	// Given a segment, the dates where a drag began and ended, calculates the Event Object's new start and end dates
/*
...
$.extend(DayGrid.prototype, {
	...
	renderHelper: function(event, sourceSeg) {
		...
		this.rowEls.each(function(row, rowNode) {
			...
			// change: user
			var user = rowEl.parents('.fc-widget-content')
					.prev('.fc-axis').data('user');
			if (user == event.user) {
				...
				if (sourceSeg && sourceSeg.row === row) {
					...
					// change: table вместо tbody
					skeletonTop = rowEl.find('.fc-content-skeleton table').position().top;
				}
				// change: всегда 1 строка на пользователя
				row = 0;

				// change: user
				skeletonEl.css('top', skeletonTop)
					.find('table')
						.append(rowStructs[event.user][row].tbodyEl);
	...
	renderHighlight: function(start, end) {
		...
		for (i = 0; i < segs.length; i++) {
			seg = segs[i];
			//change: highlight, подсветим не 1 строку, а все
			this.rowEls.each(function(j, item) {
				el = $(
					self.highlightSkeletonHtml(seg.leftCol, seg.rightCol + 1) // make end exclusive
				);
				el.appendTo(item);
	...
	destroyHighlight: function() {
		...
		*/
		if (this.highlightEls) {
			$.each(this.highlightEls, function(i, item) {
				$(item).remove();
			});
			this.highlightEls = null;
		}
...
$.extend(DayGrid.prototype, {
	...
	//change: обработка ошибки, если строк больше, чем данных
	...
	renderEvents: function(events) {
		...
		this.rowEls.each(function(i, rowNode) {
			user = $(rowNode).parents('.fc-widget-content')
					.prev('.fc-axis').data('user');
			if (rowStructs[user]) {
				for (var j = 0; j < rowStructs[user].length; j++) {
					if (rowStructs[user][j].segs.length > 0) {
						$.each(rowStructs[user][j].segs, function(k, item) {
							if (item.event.user) {
								$(rowNode).parents('.fc-view')
										.find('[data-user="' + item.event.user + '"]')
										.next()
										.find('.fc-content-skeleton > table').append(
									rowStructs[user][j].tbodyEl
								);
					...
					segs.push.apply(segs, rowStructs[user][j].segs);
...
*/
	...
	destroyEvents: function() {
		...
		//change: group
		for (var user in this.rowStructs) {
			if (!this.rowStructs.hasOwnProperty(user)) {
				continue;
			}
			rowStructs = this.rowStructs[user] || [];
	...
	renderEventRows: function(events) {
		...
		for (var user in segRows) {
			if (!segRows.hasOwnProperty(user)) {
				continue;
			}
			rowUsers[user] = [];
			for (row = 0; row < segRows[user].length; row++) {
				rowUsers[user].push(
					this.renderEventRow(row, segRows[user][row])
				);
			}
		}
	...
	renderSegHtml: function(seg, disableResizing) {
		...
		// change: resize
		var isResizable = !disableResizing && view.isEventResizable(event);// && event.allDay && seg.isEnd;
		...
		// change: уубираем лишние строки
		var shortTitle = event.title;
		if (event.title.indexOf("\n") > 1) {
			shortTitle = shortTitle.substr(0, event.title.indexOf("\n"));
		}
		titleHtml =
			'<span class="fc-title">' +
				(htmlEscape(shortTitle || '') || '&nbsp;') + // we always want one line of height
			'</span>';
...
*/
	...
	groupSegRows: function(segs) {
		...
		for (i = 0; i < segs.length; i++) {
			segUsers[segs[i].event.user] = [];
			for (j = 0; j < view.rowCnt; j++) {
				segUsers[segs[i].event.user].push([]);
			}
		}

		for (i = 0; i < segs.length; i++) {
			segUsers[segs[i].event.user][segs[i].row].push(segs[i]);
		}
		...
		while (slotTime < this.maxTime) {
			...
			html +=
				'<tr ' + (!minutes ? '' : 'class="fc-minor"') + '>' +
					(!isRTL ? axisHtml : '') +
					'<td class="' + view.widgetContentClass + ' 1"/>' +
					(isRTL ? axisHtml : '') +
				"</tr>";
...
View.prototype = {
	...
	computeScrollerHeight: function(totalHeight) {
		...
		// change: Чтобы не прыгала высота
		return totalHeight;
	...
	reportSelection: function(start, end, ev) {
		var elem = jQuery(ev.target).parents('.fc-widget-content').prev(); 
		ev.user = elem.data('user');
		ev.username = elem.text();
...
function View(calendar) {
	...
	function eventDrop(el, event, newStart, ev) {
		...
		var mutateResult = calendar.mutateEvent(event, newStart, event.end);
	...
	function eventResize(el, event, newEnd, ev) {
		...
		// change: дата окончания почему-то не передавалась
		if (event.end == null) {
			event.end = newEnd;
		}
		trigger(
	...
	function cellToCellOffset(row, col) {
		...
		//change: смещение расчитывается без учета кол-ва строк
		//var cellOffset = row * colCnt + (col * dis + dit); // column, adjusted for RTL (dis & dit)
		var cellOffset = col; // column, adjusted for RTL (dis & dit)
		return cellOffset;
	}
	...
	function dayOffsetToDate(dayOffset) {
		//change: для недели ячейку в дату переводим иначе
		if (t.name == 'irisWeek') {
			var hours = [0, 6, 12, 18];
			var hour = hours[dayOffset % 4];
			var day = parseInt(dayOffset / 4);
			return t.start.clone().add(day, 'days').add(hour, 'hours');
		}
	...
	//change: добавлен метод для расчета смещения в случае масштаба недели
	function dateToWeekPeriodOffset(date) {
		return parseInt(date.clone().diff(t.start, 'hours') / 6);
	}
	...
	function rangeToSegments(start, end) {
		...
		if (t.name == 'irisWeek') {
			var rangeDayOffsetStart = dateToWeekPeriodOffset(dayRange.start);
			var rangeDayOffsetEnd = dateToWeekPeriodOffset(dayRange.end); // an exclusive value
		}
		...
		//change: масштаб Неделя
		if (t.name == 'irisWeek') {
			if (end.minutes() == 0 && end.hours() % 6 == 0) {
				rangeCellOffsetLast--;
			}
			rangeCellOffsetLast++;
		}
		...
		for (var row=0; row<rowCnt; row++) {
			...
			if (segmentCellOffsetFirst <= segmentCellOffsetLast) {
				...
				//change: при нынешнем раскладе и так всегда sort. А иначе наоборот сбивается сортировка
	...
	function computeDayRange(start, end) {
		//change: strip
		var startDay = start.clone(); // the beginning of the day the range starts
		if (t.name != 'irisWeek') {
			startDay = startDay.stripTime(); // the beginning of the day the range starts		
		}
		var endDay;
		var endTimeMS;

		if (end) {
			endDay = end.clone(); // the beginning of the day the range exclusively ends
			//change: strip
			if (t.name != 'irisWeek') {
				endDay = endDay.stripTime(); // the beginning of the day the range starts		
...
$.extend(BasicView.prototype, {
	...
	//change: Нужно столько строк, сколько сотрудников в базе
	renderHtml: function() {
		var rows = '';
		var self = this;
		var style = 'style="width: 10%"';
		$.each(this.calendar.options.users, function(key, item) {
			rows +=
					'<tr>' +
						'<td class="fc-axis ' + self.widgetHeaderClass + '" ' + style +
							'data-user="' + item.id + '">' +
							'<span>' + // needed for matchCellWidths
							  item.name + 
							'</span>' +
						'</td>' +
						'<td class="' + self.widgetContentClass + '">' +
		...
		return '' +
			'<table>' +
				'<thead>' +
					'<tr>' +
						'<td class="fc-axis ' + this.widgetHeaderClass + '" ' + style + '>' +
							'<span></span>' +
						'</td>' +
...
*/
	...
	// A returned value of `true` signals that a mock "helper" event has been rendered.
	//change: user
	renderDrag: function(start, end, seg, user) {
		return this.dayGrid.renderDrag(start, end, seg, user);
	},
/*
	renderDrag: function(start, end, seg) {
...
$.extend(MonthView.prototype, {
	...
	render: function(date) {
		...
		colCnt = this.end.date();

		this.title = this.calendar.formatDate(this.intervalStart, this.opt('titleFormat'));

		BasicView.prototype.render.call(this, rowCnt, colCnt, false); // call the super-method
	},
/* // change: рендеринг масштаба месяца
	render: function(date) {
		var rowCnt;
...
// change: Новое представление - неделя
fcViews.irisWeek = IrisWeekView; // register the view

function IrisWeekView(calendar) {
	BasicView.call(this, calendar); // call the super-constructor
}


IrisWeekView.prototype = createObject(BasicView.prototype); // define the super-class
$.extend(IrisWeekView.prototype, {

	name: 'irisWeek',


	incrementDate: function(date, delta) {
		return date.clone().add(delta, 'weeks').startOf('week');
	},
	...
	render: function(date) {
		...
		this.intervalStart = date.clone().startOf('week');
